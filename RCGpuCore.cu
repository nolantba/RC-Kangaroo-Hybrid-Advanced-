...
__global__ void KernelA(const TKparams Kparams)
{
	...
	// ========================================================================
	// SOTA++ Herds: Calculate herd bias for spatial separation
	// ========================================================================
	u16 herd_bias = 0;
	if (Kparams.UseHerds) {
		int herd_id = kang_ind / Kparams.KangaroosPerHerd;
		herd_bias = (u16)(herd_id * 17);  // Prime number offset for herd separation
	}
	// ========================================================================
    ...
    for (int step_ind = 0; step_ind < STEP_CNT; step_ind++)
    {
        ...
        for (int group = 0; group < PNT_GROUP_CNT; group++) {
            ...
            u16 jmp_ind_loc = (X[0] + herd_bias) & (JMP_CNT - 1);  // HERD BIAS ADDED
